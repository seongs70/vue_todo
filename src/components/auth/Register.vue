...
<!--      <div v-if="successMessage" class="success-message">{{successMessage}}</div>-->
      ...
      <div class="form-control">
        ...
        <input type="email" name="email" id="email" class="login-input" :class="{'input-error':errors.has('email')}" v-model="email" v-validate="'required|email'">
      ...
      <div class="form-control mb-more">
        ...
        <input type="password" name="confirm_password" id="confirm_password" :class="{'input-error':errors.has('confirm_password')}" class="login-input" v-model="confirm_password" v-validate="'required|min:6'">
    ...
    methods: {
      register() {
        this.$store.dispatch('register', {
          ...
          .then(response => {
            this.successMessage = "등록이 완료됬습니다."
